package com.innovatech.project_service.exception;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorDTO> handleProjectException(ProjectException exception){
        if (exception.getMessage().contains("no se encuentra en la base de datos")){
            Map<String, String> errorMap = Collections.singletonMap("Proyecto no encontrada", exception.getMessage());
            return ResponseEntity.status(404)
                    .body(this.createErrorDTO(HttpStatus.CONFLICT.value(), new Date(), errorMap));
        }else{

            Map<String, String> errorMap = Collections.singletonMap("Proyecto existe", exception.getMessage());
